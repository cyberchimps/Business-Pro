...
TEMPLATE_URL . '/images/social/thumbs/icons-default.png' ), 'default' => 'default' ) )
		...
		->section("Front Page")
		->open_outersection()
			->section_order($themeslug."_front_section_order", "Drag & Drop Elements", array('options' => array("business_page_slider" => "Content Slider", "business_callout_section" => "Callout Section", "business_twitterbar_section" => "Twitter Bar", "business_index_carousel_section" => "Carousel", "business_portfolio_element" => "Portfolio","business_product_element" => "Product", "business_box_section" => "Boxes"), "default" => 'business_page_slider,business_callout_section,business_box_section'))
		->close_outersection()
		->subsection("Slider")
			...
			->select($themeslug."_front_slider_nav", "Select the Slider Navigation", array( 'options' => array("key1" => "Dots","key3" => "none")))
		...
		->subsection("Product")
			...
			->text($themeslug."_front_product_title", "Product Title", array('default' =>'Product'))
			->textarea($themeslug."_front_product_text", "Product Text")
			->select($themeslug."_front_product_type", "Media Type", array( 'options' => array("key1" => "Image", "key2" => "Video")))
			->upload($themeslug."_front_product_image", "Product Image", array('default' => array('url' => TEMPLATE_URL . '/images/pro/product.jpg')))
			->textarea($themeslug."_front_product_video", "Video Embed")
			->checkbox($themeslug."_front_product_link_toggle", "Product Link", array('default' => true))
			->text($themeslug."_front_product_link_url", "Link", array('default' => home_url()))
			->text($themeslug."_front_product_link_text", "Text", array('default' => 'Buy Now'))
		->subsection_end()
		->section("Blog")
		->open_outersection()
		...
		->subsection("Blog Slider")
			->select($themeslug.'_blog_customslider_category', 'Select the custom slide category', array( 'options' => $customslider ))
			->text($themeslug."_blog_slider_height", "Slider height", array('default' => '330'))
			->text($themeslug."_blog_slider_delay", "Slider Delay", array('default' => '3500'))
			->select($themeslug."_blog_slider_animation", "Select the Sidebar Animation", array( 'options' => array("key1" => "Horizontal-Push", "key2" => "Fade", "key3" => "Horizontal-Slide", "key4" => "Vertical-Slide")))
			->select($themeslug."_blog_slider_nav", "Select the Slider Navigation", array( 'options' => array("key1" => "Dots", "key3" => "none")))
